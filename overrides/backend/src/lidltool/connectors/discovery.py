...
def connector_discovery_payload(
    ...
    *,
    ...
    for source_id, state in lifecycle.items():
        ...
        sync_attention_reason = _sync_attention_reason(sync)
        effective_auth_state = (
            "reauth_required"
            if sync_attention_reason == "reauth_required"
            else auth_state
        )
        ...
        row = {
            ...
            "advanced": {
                ...
                "auth_state": effective_auth_state if viewer_is_admin else "hidden",
...
def _status_detail(
    *,
    ...
    sync: dict[str, Any],
    sync_attention_reason: str | None = None,
) -> str | None:
    ...
    if isinstance(block, dict):
        ...
        if summary:
            return summary
    if sync_attention_reason == "reauth_required":
        detail = _failed_sync_detail(sync)
        return detail or "Connector sign-in expired or requires human verification."
...
def _user_status(
    *,
    ...
    if sync_attention_reason == "reauth_required":
        return "needs_attention"
    if bootstrap_status == "failed" or sync_status in {"failed", "canceled"} or latest_job_status in {"failed", "canceled"}:
        return "error"
...
def _sync_attention_reason(sync: dict[str, Any]) -> str | None:
    status = (_text(sync.get("status")) or "").lower()
    latest_job = sync.get("latest_job")
    latest_job_status = (
        (_text(latest_job.get("status")) or "").lower()
        if isinstance(latest_job, dict)
        else ""
    )
    if status not in {"failed", "canceled"} and latest_job_status not in {"failed", "canceled"}:
        return None
    haystack = "\n".join(_failed_sync_lines(sync)).lower()
    reauth_markers = (
        "reauth",
        "re-auth",
        "reconnect",
    ...
    if any(marker in haystack for marker in reauth_markers):
        return "reauth_required"
    return "sync_failed"


def _failed_sync_detail(sync: dict[str, Any]) -> str | None:
    for line in reversed(_failed_sync_lines(sync)):
        candidate = _text(line)
        if not candidate:
            continue
        ...
        if not stripped:
            continue
        if stripped.lower() in {"try 'python -m lidltool.cli connectors sync --help' for help.", "usage: python -m lidltool.cli connectors sync [options]", "error"}:
            continue
        return stripped
    return None


def _failed_sync_lines(sync: dict[str, Any]) -> list[str]:
    lines: list[str] = []
    current_run = sync.get("current_run")
    if isinstance(current_run, dict):
        output_tail = current_run.get("output_tail")
        if isinstance(output_tail, list):
            lines.extend(str(line) for line in output_tail if str(line).strip())
    latest_job = sync.get("latest_job")
    if isinstance(latest_job, dict):
        error = _text(latest_job.get("error"))
        if error:
        ...
        if isinstance(summary, dict):
            ...
            if isinstance(warnings, list):
                lines.extend(str(line) for line in warnings if str(line).strip())
    return lines
