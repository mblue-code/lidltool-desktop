...
const ConnectorDiscoveryRowSchema = z.object({
  ...
  reporting_role: z.enum(["spending_only", "cashflow_only", "spending_and_cashflow"]),
...
const SourceReportingRoleUpdateSchema = z.object({
  source_id: z.string(),
  reporting_role: z.enum(["spending_only", "cashflow_only", "spending_and_cashflow"])
});
...
export type SourceReportingRole = z.infer<typeof SourceReportingRoleUpdateSchema>["reporting_role"];
...
export async function updateSourceReportingRole(
  sourceId: string,
  reportingRole: SourceReportingRole
): Promise<z.infer<typeof SourceReportingRoleUpdateSchema>> {
  return apiClient.patch(`/api/v1/sources/${sourceId}/reporting-role`, SourceReportingRoleUpdateSchema, {
