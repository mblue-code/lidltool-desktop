...
export function ChatWorkspacePage() {
  ...
  const chatUnavailable = !demoMode && !configQuery.isPending && availableModels.length === 0;
  ...
  async function handleSend(event: FormEvent<HTMLFormElement>): Promise<void> {
    ...
    if (chatUnavailable) {
      setStreamError(t("pages.chatWorkspace.error.noModelAvailable"));
      return;
    }
      ...
      <div className="grid min-h-[calc(100dvh-10rem)] gap-4 md:grid-cols-[16rem_1fr]">
        ...
        <Card className="flex min-h-[calc(100dvh-10rem)] flex-col">
          ...
          <CardContent className="flex flex-1 flex-col gap-3 py-4">
            ...
            {chatUnavailable ? (
              <Alert>
                <AlertTitle>{t("pages.chatWorkspace.unavailableTitle")}</AlertTitle>
                <AlertDescription>{t("pages.chatWorkspace.error.noModelAvailable")}</AlertDescription>
              </Alert>
            ) : null}
            {demoMode ? (
              <Alert>
            ...
            <form className="space-y-2" onSubmit={(event) => void handleSend(event)}>
              ...
              <Textarea
                ...
                rows={4}
                disabled={demoMode || streaming || chatUnavailable}
              />
              <div className="flex justify-end">
                <Button type="submit" disabled={demoMode || streaming || chatUnavailable || input.trim().length === 0 || !agent}>
