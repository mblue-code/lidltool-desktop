...
import { getDesktopOcrBridge } from "@/lib/desktop-api";
...
export function DocumentsUploadPage() {
  ...
  async function startProcessing(documentId: string): Promise<void> {
    ...
    try {
      ...
      const desktopOcrBridge = getDesktopOcrBridge();
      if (desktopOcrBridge) {
        try {
          await desktopOcrBridge.wakeOcrWorker();
        } catch (error) {
          const wakeErrorMessage =
            error instanceof Error && error.message.trim().length > 0
              ? error.message
              : t("pages.documentsUpload.processFailed");
          try {
            await failDocumentProcess(documentId, {
              jobId: result.job_id,
              errorMessage: wakeErrorMessage
            });
          } catch {
            // Preserve the original wake failure as the primary surfaced error.
          }
          throw new Error(wakeErrorMessage);
        }
      }
  ...
  useEffect(() => {
    ...
    if (isTerminalOcrStatus(statusQuery.data.status)) {
      ...
      if (statusQuery.data.status === "completed") {
        void invalidateFinanceWorkspaceQueries(queryClient);
      }
    }
  }, [activeDocumentId, queryClient, statusQuery.data, t]);
