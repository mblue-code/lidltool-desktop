...
def _merchant_family(value: str | None) -> str:
...
def _load_ground_truth() -> dict[str, Any]:
    root = _desktop_root() / "tests" / "fixtures" / "german_public_receipts_eval"
    return json.loads((root / "ground_truth.json").read_text(encoding="utf-8"))


def _selected_chains() -> set[str] | None:
    raw = os.getenv("LIDLTOOL_GERMAN_PUBLIC_CHAINS", "").strip()
    if not raw:
        return None
...
def _configure_env(config_dir: Path, documents_dir: Path) -> None:
    ...
    os.environ.setdefault(
        ...
        "desktop-german-public-eval-secret-key-1234567890abcdef",
    )
...
def _guess_mime_type(asset_path: Path) -> str:
...
def _run_receipt_eval(
    *,
    ...
    asset_path = fixture_root / str(truth["file"])
    payload = asset_path.read_bytes()
    storage_uri, sha256 = storage.store(
        file_name=asset_path.name,
        mime_type=_guess_mime_type(asset_path),
        payload=payload,
    )
    with session_scope(sessions) as session:
        document = Document(
            ...
            file_name=asset_path.name,
            ocr_status="queued",
            metadata_json={"eval_receipt_id": truth["id"], "eval_chain": truth["chain"]},
        )
    ...
    merchant_ok = _merchant_family(predicted["merchant"]) == _merchant_family(str(truth["merchant"]))
...
def _summarize_metrics(metrics: list[ReceiptMetrics]) -> dict[str, Any]:
    total_fields = len(metrics) * 3
    correct_fields = sum(
        int(metric.merchant_ok) + int(metric.date_ok) + int(metric.total_ok) for metric in metrics
    )
    expected_item_lines = sum(metric.item_expected for metric in metrics)
    matched_item_lines = sum(metric.item_matches for metric in metrics)
    predicted_item_lines = sum(metric.item_predicted for metric in metrics)
    expected_discount_lines = sum(metric.discount_expected for metric in metrics)
    matched_discount_lines = sum(metric.discount_matches for metric in metrics)
    predicted_discount_lines = sum(metric.discount_predicted for metric in metrics)
    ...
    return {
        ...
        "field_accuracy_pct": round((correct_fields / max(total_fields, 1)) * 100, 2),
        "item_recall_pct": round((matched_item_lines / max(expected_item_lines, 1)) * 100, 2),
        ...
        "overall_accuracy_pct": round((numer / max(denom, 1)) * 100, 2),
        "exact_receipt_match_pct": round(
            (sum(int(metric.exact_match) for metric in metrics) / max(len(metrics), 1)) * 100, 2
        ),
        "structured_receipt_count": sum(
            int(metric.structured_strategy == "structured_extraction") for metric in metrics
        ),
        "parser_fallback_count": sum(
            int(metric.structured_strategy == "parser_fallback") for metric in metrics
        ),
    }
...
def main() -> None:
    ...
    if Path(sys.executable).resolve() != build_python.resolve():
        raise RuntimeError(
            f"German public receipts eval must run under the build backend runtime. Expected {build_python}, got {sys.executable}."
        )

    ground_truth = _load_ground_truth()
    selected_chains = _selected_chains()
    if selected_chains:
        ground_truth = {
            **ground_truth,
            "dataset_id": f"{ground_truth['dataset_id']}-{'-'.join(sorted(selected_chains))}",
            "receipts": [
                receipt for receipt in ground_truth["receipts"] if str(receipt.get("chain")) in selected_chains
            ],
        }
    fixture_root = desktop_root / "tests" / "fixtures" / "german_public_receipts_eval"

    with tempfile.TemporaryDirectory(prefix="desktop-german-public-eval-") as tmpdir:
        root = Path(tmpdir)
        db_path = root / "german-public-eval.sqlite"
        ...
        by_chain_metrics: dict[str, list[ReceiptMetrics]] = defaultdict(list)
        for metric in receipt_results:
            by_chain_metrics[metric.chain].append(metric)

        summary = {
            "dataset_id": ground_truth["dataset_id"],
            **_summarize_metrics(receipt_results),
            "by_chain": {
                chain: _summarize_metrics(metrics) for chain, metrics in sorted(by_chain_metrics.items())
            },
            "receipts": [
                {
        ...
        out_dir = desktop_root / ".tmp" / os.getenv(
            "LIDLTOOL_GERMAN_PUBLIC_EVAL_OUTPUT_DIR",
            "german_public_receipts_chain_eval",
        )
