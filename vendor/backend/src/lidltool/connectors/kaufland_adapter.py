...
from lidltool.kaufland.client_playwright import (
    KauflandClientError,
    KauflandPlaywrightClient,
    parse_kaufland_date,
)
...
def _to_int_cents(value: Any) -> int:
    ...
    if isinstance(value, str):
        raw = value.replace("EUR", "").replace("€", "").replace(" ", "").replace(",", ".")
        if not raw:
            return 0
        try:
...
def _discount_subkind(label: str) -> str | None:
    ...
    if "coupon" in lowered or "gutschein" in lowered:
        return "coupon"
    if "aktion" in lowered or "rabatt" in lowered:
        return "promotion"
    if "treue" in lowered or "bonus" in lowered:
        return "loyalty"
    return None


class KauflandConnectorAdapter(BaseConnectorAdapter):
    required_scope_map = {
    ...
    def __init__(
        ...
        *,
        ...
        source: str = "kaufland_de",
        store_name: str = "Kaufland",
    ) -> None:
        self._client = client
        self._source = source
    ...
    def fetch_record_detail(self, record_ref: str) -> dict[str, Any]:
        ...
        if order is None:
            raise KauflandClientError(f"order not found for record_ref={record_ref}")
    ...
    def _map_order_to_receipt_payload(self, order: dict[str, Any]) -> dict[str, Any]:
        ...
        purchased_at = parse_kaufland_date(str(order.get("orderDate") or ""))
        ...
        for idx, raw_item in enumerate(items, start=1):
            ...
            if item_discount_cents != 0:
                discounts.append(
                    {
                        "type": "item_discount",
                        "promotion_id": str(raw_item.get("sku") or "kaufland_item_discount"),
            ...
            mapped_items.append(
                {
                    "name": str(raw_item.get("title") or f"Kaufland item {idx}"),
                    "qty": qty,
                    "unit": "pcs",
        ...
        for promotion in promotions:
            ...
            promo_discounts.append(
                {
                    "type": "promotion",
                    "promotion_id": "kaufland_promotion",
                    "amount_cents": discount_cents,
                    "label": str(promotion.get("description") or "Kaufland promotion"),
                    "scope": "transaction",
                }
            )
        ...
        return {
            "id": f"kaufland-{order_id}" if order_id else f"kaufland-fp-{fp[:20]}",
            "purchasedAt": purchased_at,
            "storeId": store_id,
            "storeName": self._store_name,
            "storeAddress": "kaufland.de",
