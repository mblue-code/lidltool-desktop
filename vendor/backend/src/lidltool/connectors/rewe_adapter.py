...
from lidltool.rewe.client_playwright import ReweClientError, RewePlaywrightClient, parse_rewe_date
...
class ReweConnectorAdapter(BaseConnectorAdapter):
    ...
    def fetch_record_detail(self, record_ref: str) -> dict[str, Any]:
        ...
        if order is None:
            raise ReweClientError(f"order not found for record_ref={record_ref}")
    ...
    def extract_discounts(self, record_detail: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for idx, item in enumerate(items, start=1):
            ...
            for raw_discount in item_discounts:
                ...
                subkind = "coupon" if "coupon" in source_label.lower() else None
                discounts.append(
                    {
    ...
    def _map_order_to_receipt_payload(self, order: dict[str, Any]) -> dict[str, Any]:
        ...
        purchased_at = parse_rewe_date(str(order.get("orderDate") or ""))
        ...
        for idx, raw_item in enumerate(items, start=1):
            ...
            if item_discount_cents != 0:
                discounts.append(
                    {
                        "type": "item_discount",
                        "promotion_id": str(raw_item.get("sku") or "rewe_item_discount"),
                        "amount_cents": item_discount_cents,
                        "label": "REWE item discount",
                        "scope": "item",
                    }
                )
            ...
            mapped_items.append(
                {
                    "name": str(raw_item.get("title") or f"REWE item {idx}"),
                    "qty": qty,
                    "unit": "pcs",
        ...
        return {
            "id": f"rewe-{order_id}" if order_id else f"rewe-fp-{fp[:20]}",
            "purchasedAt": purchased_at,
            "storeId": store_id,
            "storeName": self._store_name,
            "storeAddress": "rewe.de",
