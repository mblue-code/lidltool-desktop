...
class KauflandReauthRequiredError(KauflandClientError):
    pass
...
() => {
  ...
  for (const card of orderCards) {
    ...
    const promotions = [];
    const promoRegex = /((?:rabatt|coupon|gutschein|bonus|vorteil|aktion)[^0-9-]{0,80})(-?[0-9]+[.,][0-9]{2})/ig;
    let promoMatch = promoRegex.exec(text);
    while (promoMatch) {
      const description = (promoMatch[1] || "Kaufland promotion").trim();
...
class KauflandPlaywrightClient:
    def __init__(
        self,
        *,
        state_file: Path,
    ...
    def fetch_receipts(self) -> list[dict[str, Any]]:
        if not self._state_file.exists():
            raise KauflandReauthRequiredError(
                f"Kaufland session state missing: {self._state_file}. "
                "Run 'lidltool connectors auth bootstrap --source-id kaufland_de' first."
            )
    ...
    def _ensure_logged_in(self, page: Any) -> None:
        ...
        if "/login" in url or "/signin" in url:
            raise KauflandReauthRequiredError(
                "Kaufland session expired or invalid. Run 'lidltool connectors auth bootstrap --source-id kaufland_de' again."
            )


def parse_kaufland_date(value: str) -> str:
    cleaned = value.strip()
    if not cleaned:
...
_PROMOTION_RE = re.compile(
    r"(?i)(?:rabatt|coupon|gutschein|bonus|vorteil|aktion)[^0-9-]{0,80}(-?[0-9]+[.,][0-9]{2})"
)


def parse_kaufland_promotions(text: str) -> list[dict[str, Any]]:
    ...
    for match in _PROMOTION_RE.finditer(text):
        ...
        if amount <= 0:
            continue
        promotions.append({"description": "Kaufland promotion", "amount": round(amount, 2)})
    return promotions
