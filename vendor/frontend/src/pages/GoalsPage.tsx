...
function goalTypeLabel(goalType: string): string {
  if (goalType === "monthly_spend_cap") return "Monthly spend cap";
  if (goalType === "category_spend_cap") return "Category spend cap";
  if (goalType === "savings_target") return "Savings target";
  if (goalType === "recurring_bill_reduction") return "Recurring bill reduction";
  return goalType;
}
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 xl:grid-cols-[0.92fr_1.08fr]">
        <Card className="app-dashboard-surface border-border/60">
          ...
          <CardContent className="grid gap-4">
            ...
            <div className="grid gap-2">
              ...
              >
                <option value="monthly_spend_cap">Monthly spend cap</option>
                <option value="category_spend_cap">Category spend cap</option>
                <option value="savings_target">Savings target</option>
                <option value="recurring_bill_reduction">Recurring bill reduction</option>
              </select>
            </div>
            <div className="grid gap-2">
              <Label htmlFor="goal-target">Target amount (EUR)</Label>
            ...
            <div className="grid gap-2">
              ...
              >
                <option value="current_window">Current dashboard window</option>
            ...
            <div className="grid gap-2 md:grid-cols-2">
              ...
              <div className="grid gap-2">
                <Label htmlFor="goal-merchant">Merchant</Label>
            ...
            <div className="grid gap-2">
              <Label htmlFor="goal-recurring-bill">Recurring bill</Label>
              <select
        ...
        <Card className="app-dashboard-surface border-border/60">
          <CardHeader>
            <CardTitle>Goal board</CardTitle>
          </CardHeader>
                ...
                <div key={goal.id} className="rounded-[24px] border border-border/60 bg-white/70 p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">{goalTypeLabel(goal.goal_type)}</p>
                    </div>
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3 text-sm">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 font-medium">{progress?.status || "unknown"}</span>
                  </div>
                  <div className="mt-3 h-2.5 rounded-full bg-slate-100">
                    <div className="h-2.5 rounded-full bg-sky-500" style={{ width: `${percent}%` }} />
                  </div>
                  <div className="mt-3 flex flex-wrap gap-3 text-xs text-muted-foreground">
                    {goal.target_date ? <span>Target {formatDate(goal.target_date)}</span> : null}
                    {goal.category ? <span>Category {goal.category}</span> : null}
                    {goal.merchant_name ? <span>Merchant {goal.merchant_name}</span> : null}
